...
export default function SkillsForm ({ open, handleOpen, update, edit, INITIAL_FORM }: Props): JSX.Element {
  const { errorMessage, handleChange, handleSubmit, form } = useForm({ edit, INITIAL_FORM, handleOpen, update })
  ...
  return (
      <Modal open={open}>
          <div className={`${displayModal} fixed top-1/2 left-1/2 translate-y-[-50%] translate-x-[-50%] bg-gradient-to-bl from-slate-900 via-cyan-900 to-blue-900 py-12 px-16 rounded border-2 border-cyan-600`}>
            ...
            <form onSubmit={(evt) => { handleSubmit(evt) }} className={'flex flex-col gap-2 my-2 justify-center'}>
